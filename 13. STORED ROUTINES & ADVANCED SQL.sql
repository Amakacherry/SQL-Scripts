...
-- Functions can be of various types, they could be programmed manually, then they act like stored routines.
...
call employees.select_AVG_salary();
...
/* The parameter and variable outputs are identical bcos the process of implementing stored procedures 
in SQL is unique. 
...
where
 ...
 select employees.f_employees_emp_info('Aruna', 'Journel');
...
CREATE FUNCTION f_emp_avg_salary(p_emp_no INTEGER) RETURNS DECIMAL(10,2)
BEGIN
DECLARE v_avg_salary DECIMAL(10,2);  # Let's prove that the v_avg_salary cannot be accessed from outside this block
